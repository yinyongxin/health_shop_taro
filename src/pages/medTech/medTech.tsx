...
export default function MedTechPage() {
  ...
  return (
    <View className="min-h-screen bg-blue-50">
      <View className="px-5 pt-12 pb-6 bg-blue-600">
        ...
        <View className="flex items-center px-3 py-2.5 bg-blue-700/30 rounded-lg">
        ...
        <View className="flex justify-between">
          {CATEGORIES.map((cat) => (
            <View key={cat.id} className="flex flex-col items-center">
              <View className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center mb-2">
      ...
      <View className="px-4 mt-6 pb-8">
        ...
        <View className="space-y-3">
          {PRODUCTS.map((product) => (
            <View key={product.id} className="p-4 bg-white rounded-2xl shadow-sm">
              <View className="flex items-start justify-between">
                <View className="flex-1">
                  <View className="flex items-center gap-2">
                    <Text className="text-sm text-slate-800">{product.name}</Text>
                    {product.tag && (
                      <Text className="text-xs px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded">
                        {product.tag}
                      </Text>
                    )}
                  </View>
