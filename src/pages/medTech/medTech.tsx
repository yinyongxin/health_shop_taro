    ...
    <View className="min-h-screen" style={{ background: "linear-gradient(180deg, #F0F9FF 0%, #E0F2FE 100%)" }}>
      <View className="px-5 pt-12 pb-6" style={{ background: "linear-gradient(135deg, #0284C7 0%, #38BDF8 100%)" }}>
        <View className="flex items-center justify-between mb-4">
          <View>
            ...
            <Text className="text-xs text-sky-100 mt-0.5">医疗科技</Text>
          </View>
          <View className="flex items-center gap-1">
            <View className="w-2 h-2 rounded-full bg-sky-200"></View>
            <Text className="text-xs text-sky-100">在线</Text>
          </View>
        </View>
      ...
      <View className="px-4 -mt-3">
        <View className="p-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm">
          <View className="flex justify-between">
            <View>
              <Text className="text-lg font-mono text-sky-600">2,847</Text>
              <Text className="text-xs text-slate-400">在线设备</Text>
            </View>
            <View className="text-right">
              <Text className="text-lg font-mono text-sky-600">99.2%</Text>
        ...
        <View className="flex justify-between">
          {CATEGORIES.map((cat) => (
            <View key={cat.id} className="flex flex-col items-center">
              <View className="w-12 h-12 rounded-xl flex items-center justify-center mb-2" style={{ background: "linear-gradient(135deg, #E0F2FE 0%, #BAE6FD 100%)" }}>
      ...
      <View className="px-4 mt-6 pb-8">
        <View className="flex items-center mb-3">
          <View className="w-1 h-4 bg-sky-500 mr-2 rounded-full"></View>
          ...
          {PRODUCTS.map((product) => (
            <View key={product.id} className="p-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm">
              <View className="flex items-start justify-between">
                <View className="flex-1">
                  <View className="flex items-center gap-2">
                    ...
                    {product.tag && (
                      <Text className="text-xs px-1.5 py-0.5 bg-sky-50 text-sky-600 rounded">
                        {product.tag}
                      </Text>
                    )}
                  </View>
              ...
              <View className="flex items-center justify-between mt-3">
                ...
                <View className="text-xs px-3 py-1.5 bg-sky-500 text-white rounded-lg">
                  详情
                </View>
              </View>
            </View>
